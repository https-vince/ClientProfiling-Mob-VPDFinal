...
class EditCommunicationScreen extends StatefulWidget {
  ...
  const EditCommunicationScreen({Key? key, required this.principles})
      : super(key: key);
...
class _EditCommunicationScreenState extends State<EditCommunicationScreen> {
  ...
  void initState() {
    super.initState();
    // Start with at least one empty entry if blank
    final initial = widget.principles.isNotEmpty
        ? widget.principles
        : [
            {'title': '', 'body': ''}
          ];
    _titleControllers =
        initial.map((p) => TextEditingController(text: p['title'])).toList();
    _bodyControllers =
        initial.map((p) => TextEditingController(text: p['body'])).toList();
  }
        ...
        child: SingleChildScrollView(
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                const Text(
                  'Edit Communication and Customer Engagement',
                  style: TextStyle(
                    fontSize: 20,
